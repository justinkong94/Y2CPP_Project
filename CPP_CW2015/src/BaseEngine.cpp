...
void BaseEngine::StoreObjectInArray(int iIndex, DisplayableObject* pObject)
{
	...
	// adding object to stl vector
	m_ppDisplayableObjects.insert(m_ppDisplayableObjects.begin() + iIndex, pObject);
...
void BaseEngine::DestroyOldObjects()
{
	...
	// stl container contains at least one element
	if (!m_ppDisplayableObjects.empty()) {
		m_ppDisplayableObjects.clear(); // clear all elements in vector
	}
...
unsigned int BaseEngine::GetColour(int iColourIndex) const
{
	...
	case 41: return 0x1F282D; // level 1 colour
	}
...
void BaseEngine::NotifyAllObjects(int iSignalNumber)
{
	int i = 0;
	if (!m_ppDisplayableObjects.empty()) {
		// array is not null and withing vector size
		while (m_ppDisplayableObjects[i] != NULL && i < m_ppDisplayableObjects.size()) {
